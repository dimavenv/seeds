...
// Восстановление корзины покупателя, чей неоплаченный заказ удаляется.
//
...
//  - онлайн-оплата: ТОЛЬКО на возврате с оплаты ?paid=1 (ClearCartOnPaid), а
//    туда пускает лишь подписанный Robokassa Success URL; при неудаче
//    покупатель возвращается на ?failed=1 — корзина не тронута.
//
// Помощник вызывается, когда уборка удаляет зависший неоплаченный заказ
// (lib/order-cleanup.ts): состав заказа вливается обратно в серверную корзину
// вошедшего покупателя (user_store) — при следующей синхронизации товары
// вернутся, даже если корзина успела опустеть на другом устройстве. Для гостя
// корзина живёт в localStorage браузера, и с сервера её не восстановить.
// Существующие позиции не задваиваются: mergeCarts берёт большее количество.
//
// Функция «тихая»: любая ошибка попадает в лог и не прерывает уборку.
