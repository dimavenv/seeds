...
// Точки выдачи доступны через Ozon Rocket / Ozon Доставку — отдельный API.
// Чтобы не хардкодить эндпоинт (у разных договоров он различается), адрес и
// авторизация берутся из переменных окружения:
//
//   OZON_PVZ_API_URL   — полный URL метода, отдающего пункты (GET/POST)
...
// чтобы интерфейс выбора работал и его можно было посмотреть. Когда подключим
// реальный Rocket, интерфейс не меняется — меняется только этот адаптер.
