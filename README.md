...
- **Промокод `УРОЖАЙ`** в корзине: скидка на товары (по умолчанию 10%, меняется
  переменной `PROMO_HARVEST_PERCENT`). Работает только у вошедших в аккаунт и
  ровно один раз на аккаунт — учёт в коллекции `promo_uses` с уникальным
  индексом «аккаунт + код». Скидку считает сервер по ценам из БД.
