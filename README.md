...
- **Корзина и избранное** — в `localStorage` (работают без аккаунта).
...
- **Промокоды** — создаются и правятся в админке (`/admin/промокоды`, раздел
  ...
  Для `УРОЖАЙ` действует обязательное ограничение: только первый созданный заказ
  на email, включая гостевые и неоплаченные заказы. Вход не требуется.
  Вошедший покупатель оформляет заказ только на email своего аккаунта.
- **Аккаунты:** отдельной регистрации нет; после подтверждения оплаты кабинет создаётся
  автоматически и заказ сразу связывается с ним. На почту приходят логин и
  сгенерированный пароль; сменить его можно в кабинете. Восстановление — по одноразовой ссылке.
...
- **Аналитика `/admin/analytics`:** посещаемость из Яндекс.Метрики рядом с
  ...
  (корзина, оформление,
  покупка с суммой, промокод, поиск, вход, поддержка) и состав заказов
...
### Если страница всё-таки упала

Есть свои экраны ошибок: `app/error.tsx` (сбой внутри страницы — с кнопкой
«Попробовать ещё раз») и `app/global-error.tsx` (сбой самого каркаса).
Без них Next показывает покупателю системную заглушку «Application error»,
и сайт выглядит сломанным, хотя чаще всего достаточно повторить попытку.

### Что проверяется само

```bash
npm test              # 430+ тестов, включая инварианты безопасности
npm run audit:check   # уязвимости зависимостей с учётом принятых исключений
npm run scan:secrets  # секреты во всей истории git
npm run img:og        # пересобрать картинку превью ссылки (после смены логотипа)
```
...
1. Создайте пользователя в админке PocketBase: коллекция **users** → New record.
...
app/
  ...
  login/                  # вход; /register перенаправляет сюда
