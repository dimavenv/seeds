...
`last_name`, `first_name`, `middle_name` и `phone` (сайт подставляет их в
оформление заказа вместе с почтой аккаунта). Без импорта схемы форма «Мои
данные» в кабинете при сохранении честно скажет, что базе не хватает полей.
